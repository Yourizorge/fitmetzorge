...
(function(root){
 ...
 function period(type,value){const y=Number(value.slice(0,4));if(type==='year')return {from:`${y}-01-01`,to:`${y}-12-31`};let m=type==='quarter'?(Number(value.slice(-1))-1)*3+1:Number(value.slice(5,7));const end=type==='quarter'?m+2:m;return {from:`${y}-${String(m).padStart(2,'0')}-01`,to:new Date(Date.UTC(y,end,0)).toISOString().slice(0,10)};}
 function project(s,from='0001-01-01',to='9999-12-31'){
  ...
  const revenue=-s.accounts.filter(a=>a.category==='income').reduce((n,a)=>n+balance(a.code,null,from,to),0),cost=s.accounts.filter(a=>a.category==='expense').reduce((n,a)=>n+balance(a.code,null,from,to),0);
  ...
  const invoices=s.records.filter(r=>r.kind==='invoice'&&r.status==='posted'&&!r.parent_id).map(r=>{const outstanding=balance('1100',r.id),credits=s.records.filter(c=>c.kind==='invoice'&&c.status==='posted'&&c.parent_id===r.id&&c.date<=to).reduce((n,c)=>n+c.data.document.totalCents,0),total=r.data.document.totalCents;return {...r,outstanding,statusLabel:credits===-total?'Gecrediteerd':outstanding<0?'Terug te betalen':outstanding===0?'Betaald':outstanding<total+credits?'Gedeeltelijk betaald':'Openstaand'};});
  ...
  for(const r of s.records){
   if(r.kind==='invoice'&&r.status==='posted'&&r.data.document.vatMethod==='invoice'&&r.date>=from&&r.date<=to)vat.push({id:r.id,date:r.date,label:r.number,output:r.data.document.vatCents,input:0});
   if(r.kind==='expense'&&r.date>=from&&r.date<=to){vat.push({id:r.id,date:r.date,label:r.data.supplier,output:r.data.charge_vat_cents,input:r.data.input_vat_cents});flags.push(...(r.data.flags||[]).map(f=>`${r.data.supplier}: ${f}`));if(!settings(r.date))flags.push(`${r.data.supplier}: fiscale instellingen ontbreken`);}
   if(r.kind==='correction'&&r.data.expense_reversal&&r.date>=from&&r.date<=to){const e=records.get(r.parent_id);vat.push({id:r.id,date:r.date,label:'Tegenboeking '+e?.data.supplier,output:-(e?.data.charge_vat_cents||0),input:-(e?.data.input_vat_cents||0)});}
  }
  ...
  return {balance,revenue,cost,profit:revenue-cost,accounts,bank,invoices,expenses,unallocated,vat,flags:[...new Set(flags)],outputVAT:vat.reduce((n,v)=>n+v.output,0),inputVAT:vat.reduce((n,v)=>n+v.input,0)};
 }
 ...
 const api={money,cents,date,csv,mapping,hash,importRows,period,project,csvOut};if(typeof module!=='undefined')module.exports=api;else root.FMZAccountingModel=api;
