...
test('A4 preview fits all screens, enlarges privately and keeps archived download/share bytes',{timeout:180000},async()=>{
 ...
 try{
  ...
  await require('./billing-fixture.cjs').monthly(preview,p,'pt-progressie');await p.locator('[data-acc-action=new-package]').click();await require('./billing-fixture.cjs').open(p);const form=p.locator('#invoiceDraftForm');
