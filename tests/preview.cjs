 ...
 const server=http.createServer(async(req,res)=>{
  ...
  if(url.pathname==='/__test__/rpc'){
   ...
   const run=async()=>{try{
    ...
    if(name==='fmz_billing_command')return send(200,{data:(await database.as(account,'select public.fmz_billing_command($1,$2,$3) result',[params.action,JSON.stringify(params.payload),params.request_id])).rows[0].result,error:null});
  ...
  if(!file.startsWith(root+path.sep) || !['.js','.mjs','.css','.html','.png','.svg','.pdf','.csv'].includes(path.extname(file)) || relative.startsWith('node_modules'))return send(404,{});
  try{let content=fs.readFileSync(file);const type={'.js':'text/javascript','.mjs':'text/javascript','.html':'text/html','.css':'text/css','.png':'image/png','.svg':'image/svg+xml','.pdf':'application/pdf','.csv':'text/csv'}[path.extname(file)];
